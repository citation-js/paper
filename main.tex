...
To convert one data format (or scheme) to another, a \emph{crosswalk} is needed. A crosswalk is a set of mappings between equivalent properties in different formats.
...
A big part of Citation.js, other than parsing and formatting specific syntaxes, is the mappings between different formats. Since creating mappings between each formats is unnecessarily much work, CSL-JSON was chosen as a central format. To review the mappings, the number of mapped properties and types relative to CSL-JSON were counted. Additionally, some poorly defined or duplicate properties were counted. Table \ref{table:completeness} shows those results.
...
\begin{table}[bt]
\caption{\textbf{Mapping statistics.}
Number of CSL-JSON properties and types mapped to different formats, the CSL-JSON column being a total. Note that the intentions behind at least three properties are contested, the values of three other properties can usually be derived from other fields, ten properties are specific to citations and may not apply to resource-describing schemas, and sixteen properties could be reduced to just a few with linked data.}
\label{table:completeness}
\begin{tabular}{l|r|r|r|r|r|}
\cline{2-6}
                                 & \multicolumn{1}{l|}{CSL-JSON} & \multicolumn{1}{l|}{Wikidata} & \multicolumn{1}{l|}{BibTeX} & \multicolumn{1}{l|}{RIS} & \multicolumn{1}{l|}{BibJSON} \\ \hline
\multicolumn{1}{|l|}{Properties} & 78                            & 25                            & 21                          & 41                       & 20                           \\ \hline
\multicolumn{1}{|l|}{Types}      & 35                            & 31                            & 11                          & 30                       & 11                           \\ \hline
\end{tabular}
\end{table}
...
\begin{table}[bt]
\caption{\textbf{Different uses of Citation.js found in the wild.} Although several patches are the result of reports from the developers of these projects, the authors never contributed directly.}
\label{table:uses}
\begin{tabular}{|p{3cm}|p{6cm}|l|l|}
\hline
\textbf{Implementation}       & \textbf{Use}                                             & \textbf{Parsing} & \textbf{Formatting} \\ \hline
...
A solution for this would be to extend the format to allow for the missing entry type or property, assuming the specification authors agree. Otherwise, a custom extension could be made. Doing that for every future shortcoming may not be sustainable though, since it effectively creates a new poorly-supported standard to add to the mix. Choosing a different format is among the possibilities, but the authors have not found a suitable candidate yet; even Wikidata, which is intended to cover everything, is changing constantly and even if a proper specification is created, the question remains whether the bulk of the publication data is up to date.
